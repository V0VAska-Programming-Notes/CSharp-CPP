...
Также возможно указание конкретного соглашения о вызове, типа, __cdecl. Со специфичным для форточек __stdcall могут быть и портачки, поскольку и с `extern "C"` имена функций могут быть "покалечены" (mangled). Помимо __fastcall можно поэксперементировать и с опять-таки специфичной винде (точнее мелкомягким) __thiscall.
